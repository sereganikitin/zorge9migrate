	...
	if (isIE()) {
		setTimeout(function () {
			$(document).imagesLoaded(function () {
				...
				if ($(".swiper-vertical-pagination").length > 0) swiperVerticalPagination.onResize();
				if ($(".swiper-auto-height-container").length > 0) swiperAutoHieght.onResize();
				if ($(".swiper-multy-row-container").length > 0) swiperMultyRow.onResize();
				if ($(".swiper-blog").length > 0) swiperBlog.onResize();
				if ($(".swiper-swiperPresentation").length > 0) swiperPresentation.onResize();
				if ($(".swiper-demo-header-style").length > 0) swiperDemoHeaderStyle.onResize();
			});
		}, 300);
	}
